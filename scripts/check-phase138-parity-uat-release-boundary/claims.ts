...
export function checkClaims(texts: TextCorpus, failures: string[]): void {
  for (const file of D21_REQUIRED_FILES) {
    const text = texts.get(file) ?? "";
    if (!text.includes(D21_SENTENCE)) {
      failures.push(`${file}: missing required D-21 sentence ${D21_SENTENCE}`);
    }
  }
      ...
      for (const clause of claimClauses(paragraph.text)) {
        ...
        for (const topic of DENIED_OVERCLAIMS) {
          if (lower.includes(topic)) {
            failures.push(`${file}:${paragraph.startLine}: forbidden positive Phase 138 claim: ${topic}`);
          }
        }
      }
    }
