...
- Run `bun scripts/bright-builds-check.ts all` when the managed starter checker is installed. Use `.bright-builds-rules-checks.tsv` only for deliberate exact-path exceptions with a required reason; the file is user-owned and must not be generated by Bright Builds.
