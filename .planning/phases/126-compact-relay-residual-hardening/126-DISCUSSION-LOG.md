...
**Notes:** Explicitly supplied facts may contain empty slices when the live mempool and bounded extra buffer are genuinely empty. The selected design preserves the `open-bitcoin-network` / `open-bitcoin-mempool` separation.
...
**Notes:** Entropy is acquired only for compact announcements. Failure uses the existing safe fallback and cannot record compact-announced achieved-effect evidence.
...
**Notes:** Extend the Phase 124 guard for legal Phase 126 intermediate/final states, retain the generic active-milestone traceability checker, and block archive routing on any fresh genuine gap.
