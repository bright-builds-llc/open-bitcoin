...
Open Bitcoin shipped a default-off, bounded block-serving and compact-block relay boundary with BIP152 negotiation and reconstruction, authoritative shared runtime state, real compact/header/inventory transport, post-write evidence, sanitized operator surfaces, and deterministic release guardrails. The milestone completed 39/39 requirements and passed its final audit with 13/13 integration links and 11/11 end-to-end flows verified.

The shipped boundary remains deliberately narrow: public relay defaults, package relay, bloom/filter serving, public-network CI, production service operation, production full-node readiness, and production-funds wallet use remain deferred.
