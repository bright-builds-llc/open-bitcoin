# Requirements Archive: v1.4 Mainnet IBD Convergence and Peer Compatibility
...
- [x] **BLK-01**: Daemon sync requests, tracks, and bounds in-flight block downloads for selected validated headers without exceeding documented v1.4 resource limits.
- [x] **BLK-02**: Daemon sync validates and connects the first non-genesis block or configured checkpoint-adjacent block in the opt-in live-smoke path when reachable peers provide the required data.
- [x] **BLK-03**: Live-smoke evidence records the first validated block connection with peer endpoint, block hash, height, timestamp, and before/after durable status, or records a typed diagnosis when block progress is not reached.
- [x] **BLK-04**: Missing, `notfound`, malformed, invalid, duplicate, or disconnected block responses are peer-attributed and do not advance active chainstate or create duplicate connect work.

### Restart and Resume Evidence

- [x] **RESUME-01**: Operator can interrupt and restart the same v1.4 public-mainnet datadir after observed header or block progress and see sync resume from durable state without duplicating block connects.
- [x] **RESUME-02**: Live-smoke reporting can capture same-datadir before/after restart evidence for header height, block height, runtime phase, peer outcomes, and latest progress timestamp.
- [x] **RESUME-03**: Recovery guidance distinguishes transient peer incompatibility, public-network unreachability, invalid peer data, store corruption, store incompatibility, resource exhaustion, and intentional cancellation.

### Operator Evidence and Observability

- [x] **OBS-01**: Operator-facing status, dashboard, metrics, structured logs, RPC-facing blockchain info, and live-smoke snapshots agree on current header height, block height, peer compatibility state, progress signal, and latest error.
- [x] **OBS-02**: Operator can generate a redacted v1.4 support bundle or equivalent evidence packet that summarizes compatibility diagnostics, selected live-smoke reports, peer outcomes, status snapshots, metrics, logs, config sources, and store health without embedding raw sensitive data.
- [x] **OBS-03**: Operator docs provide copy-pasteable repo-local Cargo and Bazel commands for deterministic checks, manual-peer live smoke, same-datadir restart/resume review, support evidence collection, and pass/fail interpretation.

### Security and Release Claim Boundaries

- [x] **SEC-01**: Reviewer can inspect a v1.4 threat-model update covering public peer compatibility handling, header/block input, resource bounds, restart/resume evidence, report redaction, and operator-facing live evidence.
- [x] **SEC-02**: Reviewer can inspect refreshed parity and release-readiness docs that distinguish v1.4 opt-in outbound IBD progress from deferred inbound serving, transaction relay, production-funds wallet use, migration apply mode, packaging, hosted dashboard, GUI work, and unattended production-node claims.
- [x] **SEC-03**: Default repo verification remains deterministic; public-network checks stay opt-in and are documented as UAT evidence rather than part of `bash scripts/verify.sh`.
...
| BLK-01 | Phase 57 | Complete |
| BLK-02 | Phase 57 | Complete |
| BLK-03 | Phase 57 | Complete |
| BLK-04 | Phase 57 | Complete |
| RESUME-01 | Phase 58 | Complete |
| RESUME-02 | Phase 58 | Complete |
| RESUME-03 | Phase 58 | Complete |
| OBS-01 | Phase 59 | Complete |
| OBS-02 | Phase 59 | Complete |
| OBS-03 | Phase 59 | Complete |
| SEC-01 | Phase 59 | Complete |
| SEC-02 | Phase 59 | Complete |
| SEC-03 | Phase 59 | Complete |
