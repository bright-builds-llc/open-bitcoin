...
**Status:** All 39 requirements are independently verified and complete, and the canonical audit passes at 17/17 implementation phases. Phase 126 remains at 3/4 plans until `126-04-SUMMARY.md` exists; archival routing remains blocked during this legal promoted-pre-summary state.
...
- v2.1 retains its bounded, explicit, default-off block-serving and compact-relay claim with 39/39 requirements independently verified and complete. The canonical audit passes at 17/17 implementation phases; Phase 126 remains at 3/4 plans and archival routing remains blocked until the final summary exists.
...
| Scope v2.1 to block serving and compact block relay boundaries | v2.0 shipped bounded transaction relay and mempool participation, so the next safe node-participation expansion is serving validated blocks and compact-block relay before package relay, public defaults, or production full-node readiness | All 39 requirements are independently verified and promoted; Phase 126 remains at 3/4 plans until the final summary authorizes archive projection |
