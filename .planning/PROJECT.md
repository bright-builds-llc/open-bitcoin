...
| Scope v1.8 to production full-node readiness boundary | v1.7 left production-node readiness deferred, so the next safe step is defining support, upgrade, service, runbook, release-readiness, and evidence gates before any production claim | Phases 82 through 84 shipped the production claim boundary, support matrix, and upgrade/rollback policy; remaining v1.8 phases continue runbook, service, release-readiness, and guardrail work |
