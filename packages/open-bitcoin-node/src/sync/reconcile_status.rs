...
use super::{
    DurableSyncRuntime, SyncRunSummary, SyncRuntimeError, tip, types::SyncReconcileProgress,
};
...
impl DurableSyncRuntime {
    ...
    ) -> Result<Option<SyncReconcileProgressStatus>, SyncRuntimeError> {
        Ok(match progress {
            ...
            SyncReconcileProgress::BranchCompetitionAwaitingBodies { missing_count, .. } => {
                let Some((common_ancestor, branch_tip)) = self.branch_reconcile_points()? else {
                    return Ok(None);
                };
                Some(
            ...
            SyncReconcileProgress::SideBranchPreserved => {
                let Some(active_tip) = self.network.chainstate_snapshot()?.active_chain.pop()
                else {
                    return Ok(None);
                };
                let Some(branch_tip) = self.network.best_chain_entries()?.pop() else {
                    return Ok(None);
                };
    ...
    > {
        ...
        let common_prefix_len = active_chain
            ...
            .count();
        let Some(common_ancestor) = active_chain
            .get(common_prefix_len.saturating_sub(1))
            .or_else(|| active_chain.first())
        else {
            return Ok(None);
        };
        let Some(branch_tip) = best_chain.last() else {
            return Ok(None);
        };
        Ok(Some((common_ancestor.clone(), branch_tip.clone())))
    }
}
