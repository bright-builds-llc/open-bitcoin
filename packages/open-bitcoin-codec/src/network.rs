...
use crate::primitives::{write_i32_le, write_u16_be, write_u32_le, write_u64_le, Reader};
...
pub fn parse_block_locator(bytes: &[u8]) -> Result<BlockLocator, CodecError> {
    ...
    reader.finish()?;
    Ok(BlockLocator { block_hashes: hashes })
}
...
#[cfg(test)]
mod tests {
    use open_bitcoin_primitives::{InventoryType, MessageCommand, NetworkMagic};
    ...
    fn message_header_round_trips() {
        let header = open_bitcoin_primitives::MessageHeader {
            magic: NetworkMagic::MAINNET,
            command: MessageCommand::new("version").expect("valid command"),
            payload_size: 85,
            checksum: [1, 2, 3, 4],
        };

        let encoded = encode_message_header(&header);
        assert_eq!(parse_message_header(&encoded), Ok(header));
    }
    ...
    fn inventory_vector_encodes_expected_type_tag() {
        ...
        let encoded = super::encode_inventory_vector(&vector);
