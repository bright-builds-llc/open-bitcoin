...
v2.1 adds bounded, explicit, default-off block serving and compact-block relay
as `preview` behavior with deterministic local evidence and optional
...
| block serving | `preview` | Bounded, explicit, default-off behavior in the Phase 110/111 parity surfaces, runtime guide, status/observability contracts, and v2.1 release handoff. | `bash scripts/verify.sh` covers deterministic policy, request-path, evidence, and release-boundary checks. | Optional local or public-network operator review may preserve redacted aggregate evidence. | Public serving defaults, archive-node and production-scale historical serving, service operation, and production readiness remain deferred. | Separate public-default, archive-scale, service, support, and production-readiness gates. |
| transaction relay | `preview` | Bounded, explicit, default-off v2.0 transaction relay and mempool participation evidence in the Phase 100 through 108 parity surfaces and runtime guide. | `bash scripts/verify.sh` covers deterministic activation, inventory/download, admission, lifecycle, serving/fanout, recovery, operator-evidence, and release-boundary checks. | Optional local or public-network operator review may preserve redacted aggregate evidence. | Public/default/production relay and guaranteed public propagation remain deferred. | Separate public-default, long-run network, service, support, and production-readiness gates. |
| compact block relay | `preview` | Bounded, explicit, default-off behavior in the Phase 112 through 116 parity surfaces, runtime guide, aggregate operator evidence, and v2.1 release handoff. | `bash scripts/verify.sh` covers deterministic codec, negotiation, reconstruction, fallback, validation-handoff, evidence, and boundary checks. | Optional local or public-network operator review may preserve redacted aggregate evidence. | Public relay defaults, archive/production scale, package relay, production service, and production readiness remain deferred. | Separate public-default, long-run network, service, support, and production-readiness gates. |
