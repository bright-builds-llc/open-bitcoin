...
The current v2.0 and v2.1 boundaries supersede the historical all-deferred
description for transaction relay, block serving, and compact-block relay. They
provide bounded, explicit, default-off behavior with deterministic local
evidence and optional public-network operator review; only the broader
public-default, archive-scale, service/deployment, and production forms remain
deferred below.
...
| bounded, explicit, default-off v2.0 transaction relay | `preview` | v2.0 ships deterministic local activation, inventory/download, admission, lifecycle, serving/fanout, recovery, operator-evidence, and release-boundary coverage. | Preserve the bounded review contract and its deterministic Phase 100 through 108 evidence. |
| public/default or production transaction relay beyond the bounded v2.0 path | `deferred` | Guaranteed public propagation, public defaults, service operation, and production relay are not scoped. | Transaction-relay gate with public-default policy, long-run peer behavior, DoS controls, service support, and production evidence. |
